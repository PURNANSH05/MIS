#!/usr/bin/env python3
"""
Database Setup - Try common passwords
"""
...
passwords_to_try = ["2006", "postgres", "password", "", "admin", "123456"]
...
print("\n[Step 1] Finding correct PostgreSQL password...")
...
if not correct_password:
    ...
    print("\nPlease run this command to set PostgreSQL password:")
    print("\npsql -U postgres")
...
# Step 4: Connect to the medical_inventory database
print(f"\n[Step 4] Connecting to '{DB_NAME}' database...")
try:
    conn = psycopg2.connect(
        user=DB_USER,
        password=correct_password,
...
try:
    ...
    old_line = [line for line in env_content.split('\n') if line.startswith('DB_PASSWORD')][0]
    env_content = env_content.replace(old_line, f"DB_PASSWORD={correct_password}")
    
    with open(env_file, "w") as f:
        f.write(env_content)
    
    print(f"\n✓ Updated .env file with password: {correct_password}")
    print(f"\nNow you can:")
    print(f"1. Start backend: python main.py")
    print(f"2. Login in web app: admin/admin123")
    print(f"3. View database in pgAdmin")
    print(f"\nIn pgAdmin, use:")
