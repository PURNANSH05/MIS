...
const DisposeStock = () => {
  ...
  const canUse = hasPermission('dispose_stock');
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setForm((p) => ({ ...p, quantity: '', reason: '', reference_number: '' }));
      await fetchLookups();
    } catch (e2) {
      toast.error(e2?.response?.data?.detail || 'Failed to dispose stock');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="stock-page">
      <PageHeader
        title="Dispose Stock"
        subtitle="Dispose damaged/expired stock (batch-level)"
        icon={FiTrash2}
        actions={
      ...
      <Card className="stock-card">
        <form onSubmit={onSubmit} className="stock-form">
          <div className="form-grid">
            ...
            <div className="form-group">
              <label>Reason</label>
              <input className="input" value={form.reason} onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value }))} required placeholder="e.g. Expired / Damaged" />
            </div>
